...
normals  = zeros(length(node),2); % normals following Sulsky, York, using B-spline
normals2 = zeros(length(node),2); % normals just use mass gradient (Nairn, Zhang)
...
%%
% normals = mass gradient directly
...
for ie=1:elemCount
    ...
    for p=1:length(mpts)
        ...
        for i=1:length(esctr)
            id                = esctr(i);
            x                 = xx - node(id,:);
            [N,dNdx]          = getMPM2D(x,deltax,deltay);% NN(i,:)=dNdx;
            normals2(id,:)    = normals2(id,:)         + dNdx*mm;
        end
    end
end

% plot mesh, particles and normals
tol = 1e-8;
figure(2)
hold on
plot_mesh(node,element,'Q4','k-',1.);
