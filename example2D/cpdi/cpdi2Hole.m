...
%
% This example demonstrates that holes can appear in CPDI.
%
% Vinh Phu Nguyen
...
% this one does not (smaller particle domain)
node1 = [2.5 2.5;
         4.5 2.5;
         4.5 4.5;
         2.5 4.5;
...
% this one does not (to compare with sub-sampling)
...
% the last particle (top one) is moving down
velo(numelem,2) = -v;
volume0 = volume;
...
while ( t < time )
    ...
    for p=1:pCount
        ...
        input.Vp   = volume(p); % VOLUME from p or from particle domain???
    ...
    for p=1:pCount
      ...
      nodeIds = particles.elem(p,:);
      corners = particles.node(nodeIds,:);
...
save('cpdi-hole-normal-energies.mat','ta','ka','sa');
