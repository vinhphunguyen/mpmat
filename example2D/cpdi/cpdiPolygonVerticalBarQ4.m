...
% Particle domains are represented by Q4s but using the CPDI-poly
% framework to verify it.
%
...
interval     = 1;
vtkFileName  = 'bar';
vtkFileName1 = '../results/cpdi2/polygon/barGrid';
...
[pmesh]= buildGrid2D(l,l,numx2,numx2, 0);
...
particles.elem     = cell(pmesh.elemCount,1);

for p=1:pmesh.elemCount
  particles.elem{p} = pmesh.element(p,:);
end
...
for e = 1:pmesh.elemCount
  vx  = pmesh.node(particles.elem{e},1); 
  vy  = pmesh.node(particles.elem{e},2); 
  nv  = length(particles.elem{e});
  vxS = vx([2:nv 1]); 
...
figure
hold on
MaxNVer = max(cellfun(@numel,particles.elem));      %Max. num. of vertices in mesh
PadWNaN = @(E) [E NaN(1,MaxNVer-numel(E))];  %Pad cells with NaN
ElemMat = cellfun(PadWNaN,particles.elem,'UniformOutput',false);
ElemMat = vertcat(ElemMat{:});               %Create padded element matrix
patch('Faces',ElemMat,'Vertices',particles.node,'FaceColor','w'); pause(1e-6)
patch('Faces',ElemMat(trackedParId,:),'Vertices',particles.node,'FaceColor','r'); pause(1e-6)
