...
%
% Demonstration of polygonal CPDI in MPM and testing cracking with
% duplicated particle nodes.
%
...
E     = 69e3;        % Young's modulus
nu    = 0.22;        % Poisson ratio: make it 1D
...
% this mesh was created using PolyMesher and processed by my code
% 'interface-generator' to duplicate particle nodes. 

genPolyMesh = 1;

if (genPolyMesh )
  NElem = 800;
  [Node,Element,Supp,Load,P]=PolyMesher(@OneCircleDomain,NElem,40);
  
  Node(:,1) = Node(:,1) + mesh.deltax;
  Node(:,2) = Node(:,2) + mesh.deltay;

  % write the PolyMesher mesh to file so that interface-elem code can 
  % generate duplicated nodes
  file = 'Disk.poly';
  writePolyMesh(file,Node,Element);
end

if (genPolyMesh == 0)
  fnode   = fopen('Disk.nodes');
  felem   = fopen('Disk.elems');
  a       = textscan(fnode,'%f%f%f');
  b       = textscan(felem,'%f%f%f%f%f%f%f%f%f');
  b       = cell2mat(b); % note that b contains NAN for elements with n < 9!!!
  ...
  for i=1:size(b,1)
    Element{i} = b(i,2:9-sum(isnan(b(i,:)))); % get rid of NAN in b
  end
end
...
% Weilbull
fts = zeros(pCount,1);  % statistic tensile strenght
...
while ( t < 0 )
    ...
    nmomentum0(bottom,1) = 0; % fixed boundary conditions
    nmomentum0(bottom,2) = 0; % fixed boundary conditions       
    ...
    for p=1:pCount
        ...
        % determine (max) principal stresses
        stressTensor = [stress(p,1) stress(p,3);stress(p,3) stress(p,2)];
        pStress      = eigs(stressTensor); 
        pStrMax      = max(pStress);
        if (pStrMax > fts(p))
          crack(p) = 1; 
          nids     = stress(p,:)<0;
    ...
    for p=1:pCount                          % loop over particles
      ...
      if crack(p) == 0
        ...
        position(p,:) = getCentroidPolygon (particles,p);
        ...
        for c=1:length(pCorners)              % loop over corners of 'p'
          ...
          for i=1:length(esctr)
            ...
            if nmass(nid) > 0
              xc = xc + dtime*N*nvelo(nid,:)/nmass(nid);
            end
          end
