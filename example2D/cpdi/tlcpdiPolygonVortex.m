% This file implements the TL Material Point Method with CPDI-Q4 interpolation
...
stress  = zeros(pCount,2,2);              % full 1st PK stress
...
while ( t < time )
    ...
    for p=1:pCount
      ...
      shape      = getTLCPDIPolygonBasis(p,input,particles,mesh);
      esctr      = shape.node;
      ...
      for i=1:length(esctr)
        ...
        niforce(id,:)   = niforce(id,:) - volp*shape.dphi(i,:)*squeeze(stress(p,:,:))';        
    ...
    for p=1:pCount
        ...
        for i=1:length(esctr)
          ...
          dFp = dFp + vI'*dphi(i,:);         % particle gradient velocity
        end
        
        F          = dFp*dtime + reshape(deform(p,:),2,2);
        deform(p,:)= reshape(F,1,4);
        %volume(p)  = det(F)*volume0(p);
        J          = det(F);
        invF       = inv(F);
        P          = mu*(F-invF') + lambda*log(J)*I*invF' ; % 1st PK stress
        stress(p,:,:)  = P;      
    end
    ...
    % update CPDI data, TLMPM no need     
%     for p=1:pCount
%       data       = getCPDIPolygonData(p,particles,mesh);
%       nodeid{p}  = data.nodes;
%       funcW{p}   = data.wf;
...
Dt=13.5;
Wt=10.1;
Lf=17.2; Df=14.4;Wf=8.9;
error=(1/3)*( abs(Lt-Lf)/Lt +  abs(Dt-Df)/Dt +  abs(Wt-Wf)/Wt)