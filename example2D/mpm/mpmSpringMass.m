...
%
% Spring mass system with standard MPM.
%
% Vinh Phu Nguyen
...
rhos  = ms/A/L;      % density of spring (ideal: massless spring)
...
pCount   = 8;
[pGrid]  = buildGrid1D(L,pCount-1, 0);
...
omega = sqrt(ks/mm);  % natural frequency
Delta = mm*g/ks;      % spring stretch at static equilibrium
rhos  = ms/A/L0;      % density of spring
...
while ( t < time )
  ...
  nmomentumS(grid.tNodes,:) = 0;  % boundary conditions
...
ke     = 0.5*mm*(omega*Delta)^2*sin(omega*t).^2;
pe     = 0.5*ks*exact.^2;
...
set(gca,'FontSize',14)
hold on
plot(t,ke,'bo','LineWidth',1.6);
plot(t,pe,'rs','LineWidth',1.6);
...
plot_mesh(node,element,'Q4','k-',1.);
%plot_mesh(node,element(pElems,:),'Q4','cy-',2.1);
plot(xp0(:,1),xp0(:,2),'b.','markersize',20);
plot(xp(:,1),xp(:,2),'k.','markersize',20);
axis off
