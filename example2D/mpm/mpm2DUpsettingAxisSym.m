...
%
% MUSL with explicit time integration. Axisymmertic version.
% Stress vector=[sigma_xx sigma_yy sigma_xy sigma_theta];
% Hoop stress stored at the last component of stress vector.
%
...
for e = 1:pmesh.elemCount
    ...
    mass(e)    = a*rho*coords(e,1); %!!!! AXIS SYMMETRIC
end
...
body1.stress  = zeros(pCount,4);                % stress
body1.strain  = zeros(pCount,4);                % strain
body1.pstrain = zeros(pCount,4);                % plastic strain
...
body2.stress = zeros(pCount,4);                % stress
body2.strain = zeros(pCount,4);                % strain
body2.velo   = zeros(pCount,2);                % velocity
body2.velo(:,2) = -v0;

color2  = 2*ones(pCount,1); 
color   = [color1;color2];
        ...
        for ie=1:length(elems)         % loop over computational cells or elements
            ...
            for p=1:length(mpts)       % loop over particles 
                ...
                for i=1:length(esctr)  % loop over nodes of current element "ie" 
                    ...
                    niforce(id,1)   = niforce(id,1) - Vp*(stress(1)*dNIdx + stress(3)*dNIdy + N/xp(1)*stress(4));
        ...
        for ie=1:length(elems)         % loop over computational cells or elements
            ...
            for p=1:length(mpts)       % loop over particles
                ...
                for i=1:length(esctr)
                    ...
                    hoopStr = hoopStr + N*vI(1)/xp0(1);  %!!! AXISYMMETRIC 
                end
                ...
                dEpsV   = [dEps(1,1) dEps(2,2) 2*dEps(1,2) dtime*hoopStr];
                
                bodies{ib}.strain(pid,:)  = bodies{ib}.strain(pid,:) + dEpsV;
...
%                     bodies{ib}.stress(pid,:)  = bodies{ib}.stress(pid,:) + dsigma';
                    ...
                    [sigma,alpha,epsilonp] = updateVonMisesAxisSym ( epsilon,epsilonp0,alpha0,mu,kappa,k1,yield );
    ...
    if (  mod(istep-1,interval) == 0 )
        ...
        for i=1:size(bodies{1}.stress,1)
            devSig = I_dev*s1(i,1:4)';
            s1(i,5)= sqrt(  devSig(1)^2 + devSig(2)^2 +  devSig(4)^2 + 2*devSig(3)^2  );
        end
        data.stress  = [s1;s2];
    ...
    t = t + dtime;
    istep = istep + 1;
    dtime = min(mesh.deltax/vxMax,mesh.deltay/vyMax);
    nvelo0 = nvelo;
end
