...
% June 2013.
%
% August 2015: add adaptive time step
...
kappa   = lambda + mu;
yield   = 0.1*E;          % yield stress
k1      = 400;           % Hardening modulus 
...
interval     = 100;
vtkFileName  = 'mpm2DTwoDisksPlastic';
...
pstrain = zeros(pCount,3);                % plastic strain
alpha   = zeros(pCount,1);                % strain
...
for e=1:elemCount
  ...
  for p=1:length(mpts)
    pid  = mpts(p);
    vxp  = cdil + abs(vp(pid,1));
    vyp  = cdil + abs(vp(pid,2));
...
dtime = min(deltax/vxMax,deltay/vyMax);
...
while ( t < time )
  ...
  for e=1:elemCount
    ...
    for p=1:length(mpts)
      ...
      epsilon   =  eps (pid,:)';   % column vector
      epsilonp0 =  pstrain(pid,:)';
      alpha0    =  alpha  (pid);
      
      [sigma,al,epsilonp] = updateVonMisesMaterial ( epsilon,epsilonp0,alpha0,mu,kappa,k1,yield );
      
      s (pid,:)      = sigma;
      pstrain(pid,:) = epsilonp;
      alpha  (pid,:) = al;
  ...
  % VTK output
      ...
      if (  mod(istep-1,interval) == 0 )
          ...
          vtkFile = sprintf('../results/mpm/two-disks-plastic/%s%d',vtkFileName,istep-1);
  ...
  dtime = fac* min(deltax/vxMax,deltay/vyMax);
end
...
xlabel('Time')
ylabel('Energy ( x 1E-3)')
legend('kinetic','strain','total')
%set(gca,'XTick',[0 0.5 1.0 1.5 2.0])
axis([0 4 0 2600])

%savefile = 'mpm-explicit-2disks-dt002.mat';
