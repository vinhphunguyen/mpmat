...
% The grid is a structured mesh consisting of B-splines using Bezier.
%
% Two elastic disks come into contact.
% Example taken from Sulsky et al. 1994 paper.
%% Checked: correct result obtained.
%
% Vinh Phu Nguyen
...
D = elasticityMatrix(E,nu,stressState);
...
%surf    = doKRefinementSurface(surf,pnew,qnew,5,5); % k-refinement
surf    = doHPSurface(surf,pnew,qnew,5,5); % k-refinement
igaMesh = buildIGA2DMesh(surf);               % parameter mesh
vMesh   = buildVisualizationMesh2D(surf);     % physical mesh
[C,Cxi,Cet]  = bezierExtraction2D(igaMesh.uKnot,igaMesh.vKnot,igaMesh.p,igaMesh.q);
...
% body1
ghostCell = 0;
center = [0.2+deltax*ghostCell 0.2+deltay*ghostCell];
radius = 0.2;
...
% body 2
center = [0.8+deltax*ghostCell 0.8+deltay*ghostCell];
radius = 0.2;
...
s       = zeros(pCount,3);                % stressx
eps     = zeros(pCount,3);                % strain
vp      = [ones(pCount/2,2)*v;-ones(pCount/2,2)*v];% velocity
...
activeNodes = unique(igaMesh.globElems(activeElems,:));
...
figure
hold on
%plot_mesh(node1,element1,elemType,'cy-',0.2);
%plot_mesh(node,element(pElems,:),'Q4','cy-',2.1);
plot(xp(:,1),xp(:,2),'b.','markersize',10);
plot(igaMesh.controlPts(:,1),igaMesh.controlPts(:,2),'k.','markersize',20);
plot_mesh(vMesh.node,vMesh.element,'Q4','r-',0.2);
%nrbkntplot(surf);
axis off
...
while ( t < time )
    ...
    for e=1:elemCount                
        ...
        for p=1:length(mpts)
            pid  = mpts(p);
            xi   = (2*xp(pid,1)-(pts(1,1)+pts(end,1)))/deltax;
            et   = (2*xp(pid,2)-(pts(1,2)+pts(end,2)))/deltay;  
            
            [B, dBdxi] = getShapeGradBernstein2D(igaMesh.p,igaMesh.q,xi,et);
%             N          = Ce*B;
%             dNdxi      = Ce*dBdxi(:,1);
%             dNdeta     = Ce*dBdxi(:,2);
            N          = B;
            dNdxi      = dBdxi(:,1);
            dNdeta     = dBdxi(:,2);
            %[N, dNdxi, dNdeta] = BSPLINE2DBasisDers([xi;et],igaMesh.p,igaMesh.q,...
                %igaMesh.uKnot,igaMesh.vKnot);
            ...
            for i=1:length(esctr)
                ...
                nmomentum(id,:) = nmomentum(id,:) + N(i)*mp*vp(pid,:);
                niforce(id,1)   = niforce(id,1) - vol*(stress(1)*dNIdx + stress(3)*dNIdy);
                niforce(id,2)   = niforce(id,2) - vol*(stress(3)*dNIdx + stress(2)*dNIdy);
            end
        end
    end
...
%             dNdeta     = Ce*dBdxi(:,2);
            ...
            dsigma  = D * [dEps(1,1);dEps(2,2);2*dEps(1,2)] ;
            s(pid,:)  = s(pid,:) + dsigma';
