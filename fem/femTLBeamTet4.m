...
markedNode2 = find(abs(nodes(:,2)-0)<1e-10);
markedNode3 = find(abs(nodes(:,3)-0)<1e-10);

markedNode  = intersect(intersect(markedNode1,markedNode2),markedNode3);

fixNodes=find(abs(nodes(:,1)-0)<1e-10);


Node1 = find(abs(nodes(:,1)-4)<1e-10);
Node2 = find(abs(nodes(:,2)-1)<1e-10);


forceNodes1   = intersect(Node1,Node2);
forcedNodes   = forceNodes1;
...
%plot(nodes(forceNodes,1),nodes(forceNodes,2),'rs','MarkerSize',9,'MarkerFaceColor','red')
axis off
...
ss = load('femTet4Cantilever1000.mat');
...
A=[ta' pDisp];dlmwrite('filename.txt',A)
...
A = [ta pDisp'];
fileID = fopen('cantilever-FEM.txt','w');
