...
ai       = zeros(nsd^2, gpCount,mesh.elemCount); % tangent stiffness of localisation band
ai0      = zeros(nsd^2, gpCount,mesh.elemCount); % tangent stiffness of localisation band
a        = zeros(nstr^2,gpCount,mesh.elemCount); % homogenised tangent stiffness
a0       = zeros(nstr^2,gpCount,mesh.elemCount); % homogenised tangent stiffness
...
jump     = zeros(nsd,   gpCount,mesh.elemCount); % jump
jump0    = zeros(nsd,   gpCount,mesh.elemCount); % jump previous converged step
...
%       end
      ...
      if (  mod(ii-1,interval) == 0 )
        ...
        vtkFile = sprintf('%s%d','notched-beam-explicit',ipara);
        ipara=ipara+1;
...
pvdFile = fopen('notched-beam-explicit.pvd', 'wt');

fprintf(pvdFile,'<VTKFile byte_order="LittleEndian" type="Collection" version="0.1">\n');
fprintf(pvdFile,'<Collection>\n');

for i = 1:ipara
  vtuFile = sprintf('%s%d%s','notched-beam-explicit',i,'.vtu');
