...
% Displacement control to trace the equilibrium curve.
%
%
...
jump     = zeros(3,gpCount,mesh.elemCount); % inelastic strain
jump0    = zeros(3,gpCount,mesh.elemCount); % inelastic strain previous converged step
...
for i = 1:70
  ...
  if (diverged==0)
    ...
    if (isFailure==0)
      ...
      plot(rdisp(1:end-xx),rforce(1:end-xx),'black-*','LineWidth',1.);
      xlim([0,1.4]); ylim([0,1]);
      drawnow
...
set(gca,'FontSize',14)
hold on
plot(rdisp(1:end-xx),rforce(1:end-xx),'black-*','LineWidth',1.6);
xlabel('u [mm]')
ylabel('P [N]')
legend('fixed, 867 elems','fixed, 1633 elems','no fixed, 1633 elems')
axis([0 1.4 0 2])
