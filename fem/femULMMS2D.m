...
%
% Manufactured solution for the axis-aligned problem.
%
% Vinh Phu Nguyen
...
noX0      = 2^m(3);      % number of elements along X direction
...
while ( t < time )
  ...
  for e=1:elemCount
    ...
    for p=1:length(W)
      ...
      % external force due to manufactured body force, in initial
      % configuration!!!
      XX  = N'*enode0;       
      J0  = enode0'*dNdxi;                  % element Jacobian matrix
      neforce(esctr,1) = neforce(esctr,1) + rho*W(p)*det(J0)*N*mmsB1(XX(1),XX(2),t);      
      neforce(esctr,2) = neforce(esctr,2) + rho*W(p)*det(J0)*N*mmsB2(XX(1),XX(2),t);      
    end
  end
...
dispLinf2=[0.001271443657792;
           3.166521054799074e-04;
           7.908260683281833e-05;
          1.976587317429475e-05
     ];   
